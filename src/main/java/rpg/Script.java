...
/**
 * A <code>Script</code> is a generic class that can describe any of a number of
 * objects that can be
 * part of a level. Creatures, scenery, and items are all scripts, but so are
 * other things like notes
...
public class Script {
     ...
     * @param action   The action, or type, of the script. For example, "Town" or
     *                 "Creature".
     *                 Pass null to make a script that will match any action.
     * @param argument The argument of the script, giving the extra information on
     *                 how it
     *                 does its action.
     */
    ...
    /**
     * Determine whether this script matches another script. This is more
     * complicated than a simple
     * <code>equals()</code> method, because not all the instance variables will be
     * used as criteria
     * for comparison; if either <code>this</code> or <code>other</code> has -1 as
     * one of its
     * coordinates or <code>null</code> as its action or parameter, that property is
     * automatically
     * assumed to match.
     * 
     * If you are doing the "radius" extension in part 1, you will have to write
     * slightly more
     * complicated code in here to calculate the distance between the two scripts
     * and verify that
    ...
    public boolean matches(Script other) {
        if (x != other.x && x != -1 && other.x != -1) {
            return false;
        } else if (y != other.y && y != -1 && other.y != -1) {
            return false;
        } else if (action != null && other.action != null && !action.equals(other.action)) {
            return false;
        } else if (argument != null && other.argument != null && !argument.equals(other.argument)) {
            return false;
        } else {
            return true;
        }
    ...
    /**
     * Set this script to have an area effect. You can imagine it as a circle
     * centered at
     * its given point. Any other script that intersects that circle will match it.
     * 
     * This is only used by an extension in part 1. Regular scripts may be
     * considered to have
     * a radius of 0.
     * 
    ...
    /**
     * Get the radius over which this script has an effect. You only need to change
     * this method
     * if you are doing the relevant extension in part 1; regular scripts are
     * assumed to all have
     * a radius of 0.
     * 
     * @return the radius
     ...
     */
    public void move(int x, int y, Level newLevel) {
        this.x=x;
        this.y=y;
    ...
    /**
     * Move this script to the same location as the given script, in the same level
     * as that script.
     * If it is already in that level, only its location will be changed. In
     * particular, this method
     * will not change the position of this script in the script list for its level
     * if it is not,
    ...
    /**
     * This is a "factory" method, a static method whose job it is to instantiate
     * objects of a
     * class. You will use methods like this when you aren't sure whether the object
     * you return
     * will belong to this base class, or to one of its subclasses. So, for example,
     * here if I
     * create a script with type "Creature" it will really instantiate a
     * <code>Creature</code>
     * object and return it.
     * 
     * (This still agrees with the return type, because <code>Creature</code> is a
     * subclass of
     * <code>Script</code>)
     * 
